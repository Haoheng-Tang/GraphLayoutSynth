...
- `propose-instruction-variant` translates a markdown/text design-instructions file into a YAML config variant, reusing the grammar-variant assistant's prompt building and validation. Claude proposes YAML only, never graphs; invalid proposals are saved but never generate graphs. See `docs/INSTRUCTION_GUIDED_VARIANTS.md`.
