...
- `POST /suggest-next-room` responses carry additive diagnostics on every response, including empty ones: `matchedSampleCount`, `samplesWithCandidates`, and `configSource` (`mode`/`configPath`/`variantId`). They separate "no semantic anchor match" from "matched but saturated" and name the config behind a suggestion. Source them from the values the aggregator and sampler already compute — do not recompute, do not make them depend on debug artifacts, and do not treat them as application logic.
...
- Grammar rules support simple exact node-attribute matching, created-node aliases, fixed counts, min/max counts, choice sampling, matched-node updates, optional matched-node removal, and edge modes `one_to_one`, `each_to_one`, `one_to_each`, `adjacent_pairs`, `balanced_each_to_one` (the last requires a positive-integer `max_sources_per_target` and fails rule application on infeasible capacity instead of partially assigning).
