...
| `GET` | `/program-requirements/room-types` | Read-only canonical room-type catalog (`{id, displayName, generated, tier}`) derived from the active config's contract; resolves the same config source as the suggestion sampler. No feature flag. |
...
The default config exposes a medium-detail healthcare vocabulary
(`PatientRoom`, `OnStageCorridor`/`OffStageCorridor`, `NurseStation`,
clinical/staff/public support rooms, `Stair`/`Elevator`, MEP/service rooms —
see `docs/ROOM_VOCABULARY.md`). Three tiers apply: the full **allowed
canonical vocabulary** (valid for Program Requirements, the room-type
catalog, and variant prompts), the smaller **default essential generated
mix** the grammar actually produces, and **optional room types** that are
valid on request but not generated by default. Graph node types stay flat
PascalCase IDs; hierarchy is expressed only through `semantic_node_groups`.
Legacy `Corridor`, `ClinicalSupport`, and `StaffSupport` were removed from
the canonical vocabulary; circulation is declared through the `corridor`
semantic group. `Stair`/`Elevator` are graph nodes only — no
egress/code/travel-distance logic is implied.
...
- stochastic choices, such as `type: {choices: [PatientRoom, ConsultRoom]}`
...
Each candidate summary includes validity status, final score, score breakdown when available, key deterministic metrics, node and edge counts, node and edge type counts, per-type support counts plus separate per-category rollups derived from the config's `clinical_support`/`staff_support` semantic groups, graph degree statistics, typed accessibility metrics, trace metadata, artifact paths, and a wall-adjacency proxy.
...
Feature extraction includes graph and report features such as node and edge counts, type counts, degree histograms, trace rule counts, wall-adjacency proxy metrics, and typed accessibility features. For typed accessibility, features follow every config-derived typed-accessibility pair recorded in the review summaries (for the default config: `PatientRoom` to nearest `NurseStation` and `MedicationRoom`) plus per-category support-group ratios.
